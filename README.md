...
주식 분석 클라이언트(독립 실행형 에디션)는 한국 주식 시장 데이터를 분석 및 시각화하기 위해 설계된 프론트엔드 전용 React 애플리케이션입니다. 이 프로젝트는 정적 JSON 데이터 파일이나 브라우저 기반 API로 작동하는 오픈소스 독립 버전으로, 다음 용도에 적합합니다:

- **서버 인프라 없이 개인 포트폴리오 추적**
